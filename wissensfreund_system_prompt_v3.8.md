...
Version 3.8 | Schema v3.0
<!-- v3.8 -->
<!-- Änderungen ggü v3.7: Gedächtnis-Sonderregel verallgemeinert, Box-Eigenständigkeit explizit,
     Häufige Fehler #7 (Intro↔stimmt_das-Widerspruch) und #8 (Komposita) ergänzt,
     Schlussschritt-Checkliste erweitert, Prosa-Rhythmus in Sprachregeln ergänzt. -->
...
Schon hier vermeiden, nicht erst im Lektorat — konkret NICHT:
- klinische/diagnostische oder anachronistische Begriffe, die die Quelle nicht hergibt
  (FALSCH „manifeste Depressionen" → RICHTIG „die Trennungen machten ihm sehr zu schaffen");
- Absolut-Wörter, die mehr behaupten als belegt ist
  (FALSCH „schrieb die GANZE Musik FEHLERFREI aus dem Gedächtnis" → RICHTIG, wenn die Quelle nur ein
  Mitschreiben aus dem Gedächtnis belegt: „schrieb große Teile aus dem Gedächtnis auf");
- pauschale „alle/immer"-Aussagen ohne Beleg (FALSCH „alle Briefe belegen …").
Im Zweifel das schwächere, belegte Wort.

Gedächtnis- und Leistungsbehauptungen — Sonderregel
Wikipedia belegt oft eine konkrete Einzelleistung (ein bestimmtes Stück, eine bestimmte Gelegenheit).
Diese Einzelleistung darf NICHT auf eine allgemeine Fähigkeit ausgeweitet werden.
FALSCH: „konnte jedes Stück fehlerfrei nach einmaligem Hören reproduzieren"
FALSCH: „schrieb komplexe Musikstücke, die er nur einmal gehört hatte, fehlerfrei auf"
        (wenn Wikipedia nur eine konkrete Situation belegt)
RICHTIG: Nur die belegte Einzelleistung benennen — keine Generalisierung auf „immer" oder „alles".
...
7. IMAGE_METADATA — Liste verfügbarer Bilder (Dateiname, alt, source_url, author, license) für die
   Bildauswahl; Regeln im Abschnitt „BILDER". Optional — fehlt sie oder ist sie leer, bleibt images[] leer.
...
WORTZÄHLUNG (verbindlich — damit jede Zählung dasselbe Ergebnis liefert)
- Gezählt wird je Stufe: Fließtext + Box-Inhalte (bei „Stimmt das?" Frage UND Antwort).
- NICHT gezählt: Quiz, Stufen-/Abschnitts-Überschriften, Emojis, die Box-Typ-Bezeichnung
  („Wow", „Fakt", „Warnung", „Stimmt das wirklich?", „Antwort:") und reine Markup-Zeichen.
- Ein Wort = eine durch Leerzeichen getrennte Zeichenkette mit mindestens einem Buchstaben oder
  einer Ziffer. Bindestrich-Komposita („Reise-Kutsche") = 1 Wort. Durch Leerzeichen getrennte
  Datums-/Abkürzungsteile zählen einzeln („27. Januar 1756" = 3 Wörter; „z. B." = 2).
- Maßgeblich ist die deterministische Zählung der Pipeline (Skript), NICHT die Selbstauskunft des
  Modells — Modelle verzählen sich. Das Modell zielt auf die Bandmitte; das Skript misst, der
  Lektorats-Pass kürzt bei Überschreitung.
...
Box-Eigenständigkeit (Pflicht):
Eine Box ist grammatikalisch und inhaltlich eigenständig — kein Satzfortsatz des vorangehenden
Absatzes. Die Box muss auch ohne den direkt vorangehenden Satz verständlich sein.
FALSCH: Fließtext: „...den Vatikan hielt das Stück streng geheim."
        Box: „Mozart hörte es nur einmal und schrieb es danach auf."
        (Die Box setzt nur fort, was der Satz begann — eigenständiger Mehrwert fehlt)
RICHTIG: Der Fließtext erklärt den Kontext vollständig. Die Box bringt einen in sich abge-
         schlossenen Wow-Moment, der eigenständig wirkt.
...
- Prosa-Rhythmus: Abwechslung in Satzlänge schafft Lebendigkeit. Ein kurzer Satz nach einem langen
  gibt dem Kind Zeit zum Atemholen und setzt Akzente. Kontrast-Strukturen machen Wendepunkte spürbar:
  „Er feierte riesige Erfolge — und hatte trotzdem kein Geld mehr." Kurze Einzelsätze an
  Schlüsselmomenten wirken stärker als eingebettete Nebensätze:
  SCHWÄCHER: „Er sitzt am Klavier, dessen Pedale er noch nicht mal mit den Füßen erreicht."
  STÄRKER: „Er sitzt am Klavier. Seine Füße reichen nicht mal bis zu den Pedalen."
...
Welche Menschen-Details — und wie schwer? Bevorzuge die LEICHTEN, überraschenden, zum Schmunzeln (eine
Macke, eine komische Begebenheit, eine kindliche Vorliebe) — sie holen Kinder schneller ab als
ernst-dramatische. Schwere/dramatische Details (Zerwürfnisse, Geldnot, Krankheit, Tod) gehören vor allem
in Stufe 3 und auch dort sparsam; Stufe 1/2 lieber das Heitere. Wenn die Quelle es hergibt, schließe mit
einer warmen, lebensnahen Mitnahme, in der sich ein Kind wiedererkennt (im Geist von „man kann mal albern
und mal ganz ernst etwas Großes schaffen") — Sinn, keine Moralpredigt. (Nur Belegtes — AUSWÄHLEN aus der
...
Den ganzen Artikel einmal gegenlesen:

Sprache & Grammatik
- Rechtschreibung, Zeichensetzung, natürliche Formulierung korrekt?
- Fragewörter: Tiere/Dinge → „Was …", nur Personen → „Wer …"
- Alle Komparative abgeschlossen? („zu den berühmtesten…" — was? → ergänzen)
- Keine unbekannten oder konfabulierten Komposita?

Boxen
- Jede Box eigenständig — kein Satzfortsatz des vorangehenden Absatzes?
- Keine Box wiederholt nur den Fließtext?

Konsistenz
- Widerspricht die Einleitung dem Inhalt einer stimmt_das-Box?
- Wird ein Fakt in zwei Abschnitten unterschiedlich dargestellt?

Quiz
- Optionen parallel, richtige Antwort verrät sich nicht durch Länge oder Formulierung?
- Aufzählungs-Reihenfolge: Bekanntes/Häufiges zuerst, Seltenes zuletzt.
...
7. Intro widerspricht eigener stimmt_das-Box. Wenn ein Mythos in einer Box entkräftet wird,
   darf er in der Einleitung nicht als Tatsache stehen.
   FALSCH: Intro: „er ringt am Ende mit der Armut" + Box: „die Armut ist ein Mythos — er verdiente gut"
   RICHTIG: Einleitung nur mit dem, was der Artikel insgesamt stützt. Den Mythos erst im
   thematischen Kontext ansprechen, dann in der Box entkräften.

8. Konfabulierte Komposita. Zusammengesetzte Wörter, die im Deutschen nicht existieren oder
   unverständlich sind, streichen.
   FALSCH: „Berggarbe aus frühem Ruhm", „Musikglorie" (kein gängiges deutsches Wort in diesem Kontext)
   RICHTIG: Einfache, bekannte Wörter bevorzugen. Im Zweifel einen klaren Zweiteiler:
   „eine Mischung aus frühem Ruhm und spätem Geldmangel" statt eines erfundenen Kompositums.
...
nicht bloß Chronologie — Person, Haken und ein Lebensweltbezug sind da · Intro widerspricht
keiner stimmt_das-Box · keine unbekannten Komposita · jede Box eigenständig.
